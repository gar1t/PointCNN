...
| Segmentation   | ShapeNet    | `$ guild run prepare-shapenet`<br> `$ guild run train-seg-shapenet`<br> *Use `guild ls -o train-seg-shapenet -p ckpts` to list available checkpoints*<br> `$ guild run test-seg-shapenet step=<train step to check>`<br> `$ guild run evaluate-seg-shapenet` |
